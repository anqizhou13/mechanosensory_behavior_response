function [probainterest, probacontrol, numberoflarvaeinterest, numberoflarvaecontrol]=cumulativeStartleEscapeFromTrx(trx, timewindows, state)

if string(state)=="startle"
    numbersactions=[3 4 9]; % stop hunch bend
elseif string(state)=="escape"
    numbersactions=[1 2 5]; % crawl headcast backup
else
    disp('Error. Wrong state.');
    return;
end
...
for larva=1:numberoflarvae
    ...
    if isempty(states_loc)
        ...
        if sum(ismember(states_loc,numbersactions))==0
