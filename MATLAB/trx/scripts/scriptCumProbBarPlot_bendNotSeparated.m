...
newXticklabel = {'attP2>Ab1-40','attP2>Ab1-42','R20B01>Ab1-40','R20B01>Ab1-42'};
testGeno = 'R20B01'; % the genotype that is being graphed against control

%%

for i = 1:length(adresses)
file = dir(fullfile(adresses{i}, 'dataFiles_bendNotSeparated/cumulativeProbabilities.mat'));
...
root_folder = '/Volumes/TOSHIBA/MATLAB/trx/bar_plots/';

%bar plot

windows = fieldnames(genotypes{1,1});
for s = 1:length(windows)
plotTitle = strcat('Cumulative Probability:',strrep(windows(s), '_', '-'));
...
for k = 1:length(stringActions)
    ...
    for j = 1:length(genotypes)-1
        ...
        for l = index_to_compare(1):index_to_compare(end)
            result = struct;
            hits = [prob(j,k)*N(j,k) prob(l,k)*N(l,k)];
            total = [N(j,k) N(l,k)];
   ...
   % save the statistical test results
        filename = strcat(root_folder,windows(s),'_',testGeno,'_','pStatistics.mat');
