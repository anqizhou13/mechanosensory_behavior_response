...
@testset "BehaviorTags.jl" begin
    for (t, tags) in enumerate((String[], ["run"], ["hunch", "roll"]))
        ...
        if t == 1
            @test string(btags) == "24-element Tag{}"
        elseif t == 2
            @test string(btags) == "24-element Tag{\"run\"}"
        elseif t == 3
            @test string(btags) == "24-element Tag{\"hunch\", \"roll\"}"
        end
    end
end
end
...
@testset "Trxmat.jl" begin
    ...
    @test string(larvastate.tags) == "24-element Tag{\"run\", \"run_large\", \"run_strong\"}"
end
end
...
@testset "Formats.jl" begin
    ...
    alltags = [:back, :back_large, :back_strong, :back_weak, :cast, :cast_large, :cast_strong, :cast_weak, :hunch, :hunch_large, :hunch_strong, :hunch_weak, :roll, :roll_large, :roll_strong, :roll_weak, :run, :run_large, :run_strong, :run_weak, :small_motion, :stop_large, :stop_strong, :stop_weak]
    ...
    for file in ("chore_auto", "trxmat_exported", "fimtrack_manual")
        ...
        for (track, track_legacy) in zip(ret[:tracks], ret_legacy[:tracks])
            @test track.id == track_legacy.id
            @test track.alignedsteps == track_legacy.alignedsteps
            @test track.path == track_legacy.path
            @test track.fullstates == track_legacy.fullstates
        end
